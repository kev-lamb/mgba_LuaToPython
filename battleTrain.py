...
batch_size = math.ceil(memory_capacity * 0.4) # how many transitions to sample from memory during each training epoch
...
class BattleAgent():
    ...
    def train(self, state, action, reward, discount, next_state, frame):
        ...
        with torch.no_grad():
            ...
            q_target = batch.reward + discount * q_target
        # Compute the MSE loss between the predicted and target values, then average
        # over the batch
        loss = F.mse_loss(q_pred, q_target)/self.batch_size #another loss to try is Huber loss according to pytorch guide
...
async def setup(reward_function, filepath, new):
    ...
    env = await (BattleEnv() if reward_function is None else BattleEnv(reward_function=reward_function))
    ...
    if not new and filepath is not None:
        battle_net.load_state_dict(torch.load(filepath))
...
async def train(env, agent, gamma, n_steps, state_folder, filename):
    ...
    for turn in range(n_steps):
        ...
        flat_state = flatten(env.observation_space, state)
        act = agent.act(flat_state)
...
if __name__ == "__main__":
    ...
    env, device, agent, battle_net = setup(reward_function=None, filepath=args.model, new = args.new)

    if args.mode == "train":
        train(env, device, agent, args.s, args.model)
    elif args.mode == "test":
        test(filepath = args.model)