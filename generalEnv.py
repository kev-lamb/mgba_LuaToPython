# General Environment for when using both the traversal and battle agent
...
import asyncio
from battleClient import performGeneralAction, resetState, emulator_connect, getState
...
class GeneralEnv(gym.Env):
    ...
    async def step(self, action, mode):
        # Execute the given action and return the new observation, reward, done, and info
        next_state = await performGeneralAction(action, mode, self.reader, self.writer)
        done = mode is not None and next_state['Mode'] != mode

        next_observation = next_state

        # useless in the general env right now
        ...
        self.observation = next_observation['Battle']
